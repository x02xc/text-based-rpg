...
Skill::Skill(std::string n, bool magic, bool dmg, float p, float c,
			 TargetType tt)
...
// buffatk
BuffAtk::BuffAtk(std::string n, bool magic, bool dmg, float p, float c,
				 TargetType tt)
...
void BuffAtk::useSkill(Character* source, Character* target) {
	...
	std::cout << terminal::foreground(target->getHealthColor())
			  << target->getName() << terminal::reset
			  << "'s attack was raised by "
			  << ((isMagic) ? terminal::foreground(terminal::brightMagenta)
							: terminal::foreground(terminal::brightGreen))
			  << statPoints << terminal::reset << "!\n";
}

// buffdef
BuffDef::BuffDef(std::string n, bool magic, bool dmg, float p, float c,
				 TargetType tt)
	: Skill(n, magic, dmg, p, c, tt) {}
...
// debuffres
DebuffRes::DebuffRes(std::string n, bool magic, bool dmg, float p, float c,
					 TargetType tt)
	: Skill(n, magic, dmg, p, c, tt) {}
...
void DebuffRes::useSkill(Character* source, Character* target) {
	...
	std::cout << terminal::foreground(target->getHealthColor())
			  << target->getName() << terminal::reset
			  << "'s defense was raised by "
			  << ((isMagic) ? terminal::foreground(terminal::brightBlue)
							: terminal::foreground(terminal::brightRed))
			  << statPoints << terminal::reset << "!\n";
}

// damagehp
DamageHp::DamageHp(std::string n, bool magic, bool dmg, float p, float c,
				   TargetType tt)
	: Skill(n, magic, dmg, p, c, tt) {}
...
void DamageHp::useSkill(Character* source, Character* target) {
	...
	dmgPoints = target->getIsDefending()
					? (dmgStat * 0.2) * points - target->getDef() * 1.5
					: (dmgStat * 0.2) * points - target->getDef();
	...
	std::cout << terminal::foreground(target->getHealthColor())
			  << target->getName() << terminal::reset << " lost "
			  << ((isMagic) ? terminal::foreground(terminal::brightBlue)
							: terminal::foreground(terminal::brightRed))
			  << dmgPoints << terminal::reset << " HP!\n";
}

HealHp::HealHp(std::string n, bool magic, bool dmg, float p, float c,
			   TargetType tt)
	: Skill(n, magic, dmg, p, c, tt) {}
...
void HealHp::useSkill(Character* source, Character* target) {
	...
	std::cout << terminal::foreground(target->getHealthColor())
			  << target->getName() << terminal::reset << " gained "
			  << terminal::foreground(terminal::brightMagenta) << healPoints
			  << terminal::reset << " HP!\n";
}
