...
Character::Character(std::string n, int l)
	: name(n), exp(0), nextLevel(nextLevelFormula(l)),
	  expDrop(expDropFormula(l)), isAlive(true), isDefending(false) {
	stats.level = l;
...
terminal::Color Character::getHealthColor() const {
	if (stats.hp >= (0.7 * stats.maxHp)) {
		return terminal::brightGreen;
	} else if (stats.hp >= (0.3 * stats.maxHp)) {
		return terminal::brightYellow;
	} else {
		return terminal::brightRed;
	}
}
...
void Character::printInfo() const {
	...
	printClass();
	std::cout << " ) =====\n" << terminal::reset;
	std::cout << "LEVEL: " << stats.level << " (" << exp << "exp / "
			  << nextLevel << "exp)" << " | ";
	std::cout << "HP: " << stats.hp << " / " << stats.maxHp << " | ";
	std::cout << ((this->getIsMagic()) ? "Mana: " : "Stamina: ")
