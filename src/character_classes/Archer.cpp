...
Archer::Archer(const std::string& n, int level) : Character(n, level) {
	...
	unlockableSkills = {{2, &ArrowShot}, {3, &Volley}};
...
void Archer::setHpStat() {
	stats.maxHp = archerHp(stats.level);
	stats.hp = archerHp(stats.level);
}

void Archer::setResourceStat() {
	stats.maxResource = archerResource(stats.level);
	stats.resource = archerResource(stats.level);
}
...
void Archer::setResistanceStat() {
	stats.resistance = archerResistance(stats.level);
}
