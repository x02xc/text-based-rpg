...
Healer::Healer(const std::string& n, int level) : Character(n, level) {
	...
	unlockableSkills = {{2, &StrengthenDefense}};
...
void Healer::setHpStat() {
	stats.maxHp = healerHp(stats.level);
	stats.hp = healerHp(stats.level);
}

void Healer::setResourceStat() {
	stats.maxResource = healerResource(stats.level);
	stats.resource = healerResource(stats.level);
}
...
void Healer::setResistanceStat() {
	stats.resistance = healerResistance(stats.level);
}
