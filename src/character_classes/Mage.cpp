...
Mage::Mage(const std::string& n, int level) : Character(n, level) {
	...
	unlockableSkills = {{2, &WeakenResistance}};
...
void Mage::setHpStat() {
	stats.maxHp = mageHp(stats.level);
	stats.hp = mageHp(stats.level);
}

void Mage::setResourceStat() {
	stats.maxResource = mageResource(stats.level);
	stats.resource = mageResource(stats.level);
}
...
void Mage::setResistanceStat() {
	stats.resistance = mageResistance(stats.level);
}
