...
Warrior::Warrior(const std::string& n, int level) : Character(n, level) {
	...
	unlockableSkills = {{2, &StrongAttack}, {3, &Warcry}};
...
void Warrior::setHpStat() {
	stats.maxHp = warriorHp(stats.level);
	stats.hp = warriorHp(stats.level);
}

void Warrior::setResourceStat() {
	stats.maxResource = warriorResource(stats.level);
	stats.resource = warriorResource(stats.level);
}
...
void Warrior::setResistanceStat() {
	stats.resistance = warriorResistance(stats.level);
}
